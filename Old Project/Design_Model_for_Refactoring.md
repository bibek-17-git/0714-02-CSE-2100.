...
GtkWidget    *g_statusLabel;
GtkListStore *g_fileQueueList;
GtkWidget    *g_fileQueueView;
GtkWidget    *g_destPathEntry;
...
src/rendering/window.c       (GTK window, dialogs, callbacks)
src/utils/utils.c            (shared helpers)

include/settings.h           (BackupSettings + load/save prototypes)
include/window.h             (GTK global widget externs + UI_CreateMainWindow prototype)
...
**Benefits:**
- **Separation of concerns**: `window.c` knows nothing about file copying
...
window.c   ──► window.h, settings.h, utils.h, backup.h, fileops.h
backup.c      ──► backup.h, settings.h, window.h, utils.h
fileops.c     ──► fileops.h, settings.h, window.h, utils.h
...
| `window.c` | Construct and manage all GTK widgets and dialogs |
...
// window.c needs everything for the settings dialog
#include "settings.h"
```
...
// window.c includes backup.h (interface) — NOT backup.c (implementation)
...
**Where:** `UI_CreateMainWindow()` in `window.c`
...
```c
// Public facade (window.h)
void UI_CreateMainWindow(void);

// Private helpers inside window.c (not in header)
...
**Where:** Button → callback connections in `window.c`

GTK's signal/callback system IS the Observer pattern. `window.c` (Subject) notifies observers (callbacks in `backup.c`, `fileops.c`) without knowing their implementation.

```c
// Observer registration (window.c)
g_signal_connect(backupButton, "clicked", G_CALLBACK(Backup_Perform),       NULL);
...
// window.c knows ONLY the function signatures — not the implementations
```
